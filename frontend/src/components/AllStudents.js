...
useEffect(() => {
    ...
    const editStudent = (stdntid) => {
       ...
       axios.put(`http://localhost:5000/student/update/${stdntid}`,newStudent).then(()=>{
                     ...
                     {students.map((std) => {
                        return <tr key={std._id}>
                             {mode === EDIT_MODE && id === std._id? (
                                            <>
                                             <td> <input type='text' value={name} onChange={e => setName(e.target.value)} placeholder={ std.name }/></td>
                                            <td> <input type='numeric' value={age} onChange={e => setAge(e.target.value)} placeholder={ std.age }/></td>
                                            <td> <input type='text' value={gender} onChange={e => setGender(e.target.value)} placeholder={ std.gender }/></td>
                                            </> 
                                            ) : (
                                            <>
                                            <td>{std.name}</td>
                                    ...
                                    <td className="edit-dlt">
                                        {mode === EDIT_MODE && id === std._id? (
                                        <button className="btn" onClick={() => editStudent(std._id)}> Save </button>
                                        ) : (
                                            <>
                                            <button className="btn" onClick={() => activeEditMode(std._id)}> Edit </button>
                                            &nbsp;&nbsp;
